...
def nega_loss_calcu(p,q,sample_k):
    p = torch.cat([p.unsqueeze(0)] * sample_k, dim=0)
    loss=-torch.log(torch.div(torch.tensor(1.0),L2(p,q)+torch.tensor(1.0)))
    return loss
...
def inter_class_loss(feature,target,config,device):
    ...
    for i in range(config.num_classes):
        ...
        if len(classindexs) != 0:
            avgfeature = np.mean(numpyfeature[classindexs * config.sample_k], axis=0)
