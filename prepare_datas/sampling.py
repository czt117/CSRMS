...
rho_0 = 0.95  # vigilance parameter
rho_1=0.95
rho_2=0.65

cluster_path = './clustering_results/art/vireo'+str(rho_0)+'_/'
...
data = np.loadtxt('./feats/vireo/feat.txt')
label = np.loadtxt('./feats/vireo/label.txt')
...
for i in range(num_data):
    label_i=label[i]                            #--图片的类别：例如：0
    class_i = np.where(label == label[i])[0]    #--数据集里面和图片同样类别的数据的index
    sample_candidates = []
    max_cluster_index=-1                        #--包含该类别图片的所有cluster中，最大的cluster在所有cluster中的index
    max_cluster_number=-1                       #--包含该类别图片的所有cluster中，最大的cluster里包含数据的个数
    for j in range(J):
...
# breakpoint()
np.save('./samples/vireo/posi_samples.npy', np.array(posi_indexs))
...
np.save('./samples/vireo/nega_samples.npy', np.array(nega_indexs))
    ...
    for key, value in class_and_number_j.items():
        ...
        if ratio<=(1-rho_1):
            ...
            for x in cluster_j:
                if label[x]==key:
                    e_m_h[x]=1
np.save('./samples/vireo/easy_mid_hard.npy', np.array(e_m_h))
...
np.save('./samples/vireo/cluster_weights.npy', np.array(cluster_weights))
...
for c in range(num_class):
    ...
    centers.append(center)
np.save('./samples/vireo/class_prototypes.npy', np.array(centers))
